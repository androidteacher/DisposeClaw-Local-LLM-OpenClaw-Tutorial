...
async def generate_image_file(req: GenerateRequest):
    """Generate an image and return the raw PNG file directly."""
    if pipeline is None:
        raise HTTPException(status_code=503, detail="Model is still loading. Please wait.")
    ...
    logger.info(f"Generating image file for prompt: '{prompt}'")
    start = time.time()
    ...
    elapsed = round(time.time() - start, 2)
    logger.info(f"Image file generated in {elapsed}s")

    return StreamingResponse(
        buffer,
        media_type="image/png",
        headers={"X-Elapsed": str(elapsed)},
    )
